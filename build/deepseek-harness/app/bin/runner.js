...
function readJsonBody(request) { return new Promise((resolve, reject) => { let body = ''; request.on('data', chunk => { body += chunk; if (body.length > 65536) request.destroy(); }); request.on('end', () => { try { resolve(body ? JSON.parse(body) : {}); } catch (error) { reject(new Error('请求格式错误')); } }); request.on('error', reject); }); }
function managerLogPath() { return path.join(VAR_DIR, `${process.env.TRIM_APPNAME || 'deepseek-harness'}.log`); }
function readManagerLog(limit = 300) { try { const lines = fs.readFileSync(managerLogPath(), 'utf8').trimEnd().split('\n'); return lines.slice(Math.max(0, lines.length - limit)).join('\n'); } catch { return ''; } }
function listManagerPlugins() { let manifest = {}; try { manifest = JSON.parse(fs.readFileSync(pluginProfilePath('package.json'), 'utf8')); } catch {} return Object.entries({ ...(manifest.dependencies || {}), ...(manifest.devDependencies || {}) }).map(([name, spec]) => { let metadata = {}; try { metadata = JSON.parse(fs.readFileSync(pluginProfilePath('node_modules', name, 'package.json'), 'utf8')); } catch {} return { name, version: metadata.version || String(spec).replace(/^file:.*[/\\]/, ''), description: metadata.description || '' }; }); }
function runManagerPluginCommand(operation, target) { const validTarget = /^[a-z0-9@/_ .:-]+$/i; if (!target || !validTarget.test(target)) return Promise.reject(new Error('插件名或安装地址无效')); const args = ['plugin', '--profile', DSH_PLUGIN_PROFILE, operation === 'install' ? 'add' : 'remove', target]; return new Promise((resolve, reject) => { const child = spawn(NODE_BIN, [DSH_BIN, ...args], { cwd: WORKSPACE_DIR, env: { ...process.env, PATH: `${path.join(APP_DIR, 'bin')}:${process.env.PATH}`, HOME: WORKSPACE_DIR } }); let output = ''; child.stdout.on('data', chunk => { output += chunk; }); child.stderr.on('data', chunk => { output += chunk; }); child.on('close', code => { if (code === 0) resolve(); else reject(new Error(output.trim() || `插件命令退出码 ${code}`)); }); child.on('error', reject); }); }
let managerBusy = false;
function handleManagerApi(request, response) { const requestUrl = new URL(request.url, `http://127.0.0.1:${PROXY_PORT}`); const route = requestUrl.pathname.replace(/^\/api\/manager/, ''); const method = request.method.toUpperCase(); Promise.resolve().then(async () => { if (route === '/status') { const alive = Boolean(dshProcess && dshProcess.pid && !dshProcess.killed); let cpu = '0.0%'; let memory = '-'; if (alive) { try { const usage = spawnSync('ps', ['-p', String(dshProcess.pid), '-o', 'pcpu=,rss=']); const fields = usage.stdout.toString().trim().split(/\s+/); cpu = `${Number(fields[0] || 0).toFixed(1)}%`; memory = `${(Number(fields[1] || 0) / 1024).toFixed(1)} MB`; } catch {} } let version = process.env.TRIM_APPVER || ''; try { version = JSON.parse(fs.readFileSync(path.join(APP_DIR, 'node_modules', '@deepseek-ai', 'dsh', 'package.json'), 'utf8')).version; } catch {} return { running: alive, pid: alive ? dshProcess.pid : null, uptime: alive ? Math.floor(process.uptime()) : 0, cpu, memory, version }; } if (route === '/plugins' && method === 'GET') return { ok: true, plugins: listManagerPlugins() }; if ((route === '/plugins/install' || route === '/plugins/uninstall') && method === 'POST') { if (managerBusy) throw new Error('已有插件操作正在执行'); const body = await readJsonBody(request); managerBusy = true; try { await runManagerPluginCommand(route.endsWith('/install') ? 'install' : 'uninstall', body.spec || body.name); return { ok: true, message: route.endsWith('/install') ? '插件安装完成' : '插件卸载完成' }; } finally { managerBusy = false; } } if (route === '/logs' && method === 'GET') return { ok: true, content: readManagerLog() }; if (route === '/logs' && method === 'DELETE') { try { fs.writeFileSync(managerLogPath(), ''); } catch {} return { ok: true, message: '运行日志已清空' }; } throw new Error('接口不存在'); }).then(body => sendJson(response, 200, body)).catch(error => sendJson(response, 400, { ok: false, message: error.message })); }
// 创建透明反代服务 (0.0.0.0:3080)
const proxyServer = http.createServer((clientReq, clientRes) => {
    if (clientReq.url === '/admin' || clientReq.url === '/admin/' || clientReq.url.startsWith('/admin?') || clientReq.url.startsWith('/admin/?')) {
        clientRes.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
        return clientRes.end(fs.readFileSync(path.join(APP_DIR, 'bin', 'manager.html')));
    }
    if (clientReq.url === '/api/manager/status' || clientReq.url.startsWith('/api/manager/')) {
        return handleManagerApi(clientReq, clientRes);
    }
    const headers = {
        ...clientReq.headers,
